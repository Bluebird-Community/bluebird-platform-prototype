...
@Testcontainers
public class AlarmdTest {

    // NOTE: this is here for now, but should be refactored so it is easier reusable by other tests
    @Container
    ...
    @Autowired
    private OpennmsEventDispatcher eventDispatcher;
...
//
   ...
   @BeforeEach
   void beforeEach() {
       Assertions.assertThat(alarmRepository.findAll()).isEmpty();
   ...
   void tearDown() {
    ...
    public void testPersistAlarm() throws Exception {
        ...
        final var node = new OpennmsNodeRef("1");
        sendNodeDownEvent("%nodeid%", node);
//        await().atMost(1, SECONDS).until(allAnticipatedEventsWereReceived());
        Assertions.assertThat(alarmRepository.findAll()).hasSize(1);
        Assertions.assertThat(eventRepository.findAll()).hasSize(1);
...
//        await().atMost(1, SECONDS).until(allAnticipatedEventsWereReceived());
        Assertions.assertThat(alarmRepository.findAll()).hasSize(1);
        Assertions.assertThat(eventRepository.findAll()).hasSize(2);

        // this should be a new alarm because of the new key
        // there should now be 2 alarms
        sendNodeDownEvent("DontReduceThis", node);
//        await().atMost(1, SECONDS).until(allAnticipatedEventsWereReceived());
        Assertions.assertThat(alarmRepository.findAll()).hasSize(2);
        Assertions.assertThat(eventRepository.findAll()).hasSize(3);
...
//    }
//
    // TODO MVR at the moment it is not possible to sent events via Kafka defining custom alarm data as that does not make sense
    // Instead it should be defined individually. Sending events and defining alarms.
    private void sendNodeDownEvent(final String reductionKey, final OpennmsNodeRef nodeRef) {
       final var event = OpennmsEventBuilder.newNodeDownBuilder("Test", nodeRef)
               .withLogDest("logndisplay")
               .withLogMessage("testing")
               .build();
       eventDispatcher.sendNow(event);
//        EventBuilder event = MockEventUtil.createNodeDownEventBuilder("Test", node);
//
//        if (reductionKey != null) {
//            AlarmData data = new AlarmData();
