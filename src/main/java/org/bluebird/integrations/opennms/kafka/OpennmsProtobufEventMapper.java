...
import org.bluebird.platform.domain.model.SnmpInfo;
...
@Component
@AllArgsConstructor
// TODO MVR this should probably only be available if integration is enabled :)
...
public class OpennmsProtobufEventMapper {
    ...
    private final OpennmsEventRepository opennmsEventRepository;
    private final OpennmsEventTemplateRenderer opennmsEventTemplateRenderer;
...
//                .withTime(event.getTime() == 0 ? null : toLocalDateTime(event.getTime()))
        ...
        return eventDefinition
                ...
                .map(definition -> {
                    ...
                    final var originalEvent = opennmsEventRepository.getReferenceById(Integer.parseInt(dto.getRef()));
                    final var context = new OpennmsEventRenderContext(dto, originalEvent);
                    final var key = opennmsEventTemplateRenderer.render(context, keyTemplate);
